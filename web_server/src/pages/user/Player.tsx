...
export default function Player() {
  ...
  useEffect(() => {
    const fetchGameDetails = async () => {
      ...
      if (data && !error) {
        ...
        setRomData(data.rom_url || data.rom_filename);
      } else {
        setStatus("error");
      }
    };
  ...
  useEffect(() => {
    const bootEmulator = async () => {
      ...
      try {
        ...
        console.error("Failed to boot WebAssembly core:", err);
        setStatus("error");
      }
    };
  ...
  // 5. Fetch Game Reactions
  const fetchReactions = useCallback(async () => {
    if (!id || id.endsWith(".nes")) return;
    const { data, error } = await supabase
      .from("likes")
  ...
  const handleReaction = async (isLike: boolean) => {
    if (!currentUser) return alert("Please sign in to react to this game!");
  ...
  const handleCommentReaction = async (commentId: string, isLike: boolean) => {
    if (!currentUser) return alert("Please sign in to react to comments!");

    try {
      const targetComment = comments.find((c) => c.id === commentId);
      if (!targetComment || targetComment.user_id === currentUser.id) return;
      ...
      <div className="w-full max-w-5xl flex flex-col mb-6">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 bg-synth-surface px-4 py-2 rounded-full border border-synth-border">
            <div
              className={`w-2.5 h-2.5 rounded-full ${status === "playing" ? "bg-green-500 shadow-[0_0_10px_#22c55e]" : status === "error" ? "bg-red-500" : "bg-yellow-500 animate-pulse"}`}
            ></div>
                  ...
                  <div key={comment.id} className="flex gap-4 group">
                    ...
                    <div className="flex-grow">
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() =>
                            handleCommentReaction(comment.id, true)
                          }
                          disabled={currentUser?.id === comment.user_id}
                          className={`flex items-center gap-1.5 text-xs font-medium transition-all ${currentUser?.id === comment.user_id ? "text-gray-600 opacity-50 cursor-not-allowed" : cUserReaction === true ? "text-synth-primary" : "text-gray-500 hover:text-white"}`}
                        >
                          <ThumbsUp
                        ...
                        <button
                          ...
                          disabled={currentUser?.id === comment.user_id}
                          className={`flex items-center gap-1.5 text-xs font-medium transition-all ${currentUser?.id === comment.user_id ? "text-gray-600 opacity-50 cursor-not-allowed" : cUserReaction === false ? "text-red-400" : "text-gray-500 hover:text-white"}`}
                        >
                          <ThumbsDown
